...
const buildConfig = () => {
  ...
  const configTemText = fs.readFileSync(configTemPath, "utf-8")
    .replace(
      'sidebar,',
      `sidebar: ${JSON.stringify(sidebarList, null, 2)},`
    )
    .replace(
      'nav,',
      `nav: ${JSON.stringify(navbarList, null, 2)},`
    );
  fs.writeFile(
    './.vitepress/config.js',
    configTemText,
    { encoding: 'utf-8' },
    err => { if(err) console.log('write file err: ', err); }
  )
};
...
const buildNavbar = (dirs) => {
  // 当前固定 引导坐标为 1 (数组下标)
  const GUIDE_INDEX = 1;
  const GITEE = 'https://gitee.com/cxsuay';
  const GITHUB_URI = 'https://github.com/cxsuay';
  const _navbarList = [
    { text: '首页', link: '/' },
    {
      text: '引导',
