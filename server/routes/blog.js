...
module.exports = router => {
  ...
  router.get('/blog/list', async (ctx, next) => {
    ...
    try {
      ...
      const blogList = await setListPage(Blog, queryParams, { pageNo, pageSize, orderBy: {
        isTop: -1,
        updateDate: 1
      } });
      ...
      const list = blogList.map(_blog => {
        const tagInfo = tagList.find(({ _id }) => objIDToStr(_id) === objIDToStr(_blog.tagID));
        const _blogItem = _blog.toObject();
        _blogItem.tagInfo = tagInfo;
